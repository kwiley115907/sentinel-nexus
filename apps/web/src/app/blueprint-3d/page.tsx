...
import { takePendingBuilding } from "@/lib/aiHandoff";
...
export default function Blueprint3DPage() {
  ...
  // Picks up a building generated by the global Sentinel AI chat (in the
  // sidebar on every page, not just this one) - it can't touch this page's
  // state directly since they're separate component trees, so it stashes
  // the generated model and routes here; this applies it once on arrival.
  useEffect(() => {
    const pending = takePendingBuilding();
    if (!pending) return;

    const { model, prompt } = pending;
    setRooms(model.rooms as Room[]);
    setWalls(model.walls as Wall[]);
    setDoors(model.doors as Door[]);
    setWindows(model.windows as WindowItem[]);
    setStairs(model.stairs);

...
export default function CadScene({
  ...
  return (
    <>
      ...
      {doors.filter((door: any) => visibleLayers[door.layer]).map((door: any) => (
        <DoorRenderer key={door.id} door={door} selected={selectedId === door.id} onSelect={() => setSelectedId(door.id)} />
      ))}

      {windows.filter((windowItem: any) => visibleLayers[windowItem.layer]).map((windowItem: any) => (
